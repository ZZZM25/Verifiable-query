...
def search_in_csv_single_pass(file_path, queries):
    ...
    with open(file_path, 'r', newline='') as csv_file:
        ...
        for row in reader:
            ...
            if i >= len(queries):
                break
            if row and row['owner_address']!='' and row['to_address']!='' and row['owner_address']!=row['to_address']:
                while row == queries[i]:
                    # print('q', queries[i])
                    # print(queries[i])
                    i = i + 1
...
name = ['block_num', 'index', 'signature', 'txID','amount', 'owner_address', 'to_address', 'expiration', 'timestamp', 'raw_data_hex']


G = nx.MultiDiGraph()
with open('E:\\code\\jiaoben2\\tocsv_only_tx\\23123\\tron100.csv', encoding='utf-8', newline='') as cs:
...
ttttt=0
df = pd.read_csv('E:\\code\\jiaoben2\\tocsv_only_tx\\23123\\tron100.csv', encoding='utf-8')
    ...
    for line_number1, line in enumerate(file1, start=1):
        ...
        if line_number1<=l1:
            ...
            if start_node not in G:
                    ...
                    for i in range(len(path) - 1):
                        ...
                        if edge_attrs_dict is not None:
                            for edge_attrs in edge_attrs_dict.values():
                                ...
                                edge_attr_block_number = int(edge_attrs['block_num'])  # 假设它应该是整数
                                edge_attr_transaction_index = int(edge_attrs['index'])  # 假设它应该是整数
                ...
                sorted_edge = sorted(edge, key=lambda x: (int(x['block_num']), int(x['index'])))
                ...
                file_path = 'E:\\code\\jiaoben2\\tocsv_only_tx\\23123\\tron100.csv'
