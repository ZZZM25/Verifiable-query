...
df = pd.read_csv('E:\\code\\jiaoben\\py111\\window1000.csv', encoding='utf-8')
    ...
    for line_number1, line in enumerate(file1, start=1):
        ...
        if line_number1 <= l1:
            ...
            if start_node not in G:
                ...
                sorted_points = sorted(list(list_tx_set), key=lambda point: (point[0], point[1]))
                # print(sorted_points)

                print(len(sorted_points))

                file_path = 'E:\\code\\jiaoben\\py111\\window1000.csv'
                results = search_in_csv_single_pass(file_path, sorted_points)
                print(len(results))
